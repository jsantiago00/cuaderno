...
**PWA / deploy**: `vite.config.js` has `base: '/soltarte/'` hardcoded for GitHub Pages project-site hosting — must be updated (and `start_url`/`scope` in the same file) if the repo is ever renamed. Icons are pre-generated static files in `public/`, not built from `favicon.svg` automatically — re-run `npm run generate-icons` after changing the source SVG. `.github/workflows/deploy.yml` builds and deploys to GitHub Pages on push to `main`, reading the 6 `VITE_FIREBASE_*` values from repo secrets.
